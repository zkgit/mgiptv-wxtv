define(['angular',  'size', 'fun'], function(angular, tpl) {
	...
	function controller($scope, $rootScope, $http) {
		...
		$scope.getCollectList = function() {
			$http.post(webset.testapiurl + 'user/oftenReadChannelList/'+$scope.openId+'.json', {}).success(function(res) {
		...
		$scope.commitdel = function(channelId, m,channelName,f) {
			$http.post(webset.testapiurl + 'user/deleteoftenWatchChannel/'+$scope.openId+'/'+channelId+'.json?channelName='+channelName+'&delAll='+$scope.delAllnum, {}).success(function(res) {
