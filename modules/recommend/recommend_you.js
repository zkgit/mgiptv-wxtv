define(['angular','size', 'fun'], function(angular, tpl) {
	...
	// 	}
		...
		$scope.getAjax = function(eq, ids) {
			if(ids == "mix") {
				$http.post(webset.testapiurl + 'vod/myvideo.json?token='+$scope.token+'&dtid='+$scope.pinyinCity+'&pageNo='+$scope.seting.pageNo+'&pageSize=15&index=1&appKey=34DB874AF269B539&appScrect=40',  {}).success(function(res) {
				...
				$scope.bs.isc = false;
				var ituUrl = webset.testapiurl + 'vod/mytypetag.json?token='+$scope.token+'&dtid='+$scope.pinyinCity+'&pageSize=6&pageNo=1&type='+ids;
